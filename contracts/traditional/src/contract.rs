...
use crate::msg::{DetailsResponse, ExecuteMsg, InstantiateMsg, MotusByAddressResponse, PaymentVerificationResponse, QueryMsg, RenewalMsg, TerminateMsg, WithdrawMsg};
...
use crate::utility::{calculate_renewal_termination_date, calculate_termination_date};
...
pub fn create_policy(
    ...
    println!("Creation Policy Started...: ");
  
    let state = STATE.load(deps.storage)?;

    // Ensure that the sender is the owner of the contract
    if state.policy_holder.to_string() != msg.policy_holder.to_string() {
...
pub fn execute_withdraw_premium(
    ...
    let msg = BankMsg::Send {
        to_address: state.policy_holder.to_string(),
