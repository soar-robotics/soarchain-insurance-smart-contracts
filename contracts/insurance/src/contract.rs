...
use crate::utils::{calculate_avg_rpm, calculate_avg_vss, calculate_mileage};

// Version info for migration
const CONTRACT_NAME: &str = "crates.io:insurance-contract";
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::CreateMileageBasedPolicy(msg) => create_pay_per_mileage_policy(deps, env, msg, info ),
        ExecuteMsg::CreateUsageBasedPolicy(msg) => create_pay_per_usage_policy(deps, env, msg, info ),
...
pub fn create_policy(
    ...
    if state.policy_holder != msg.policy_holder.to_string() {
        return Err(ContractError::Unauthorized {});
    }

    let policy = Policy::create(
        msg.id.to_string(),
        msg.policy_holder.to_string(),
        msg.insured_party.to_string(),
    ...
    POLICES.update(deps.storage, &msg.id, |existing| match existing {
        None => Ok(policy),
...
pub fn create_pay_per_mileage_policy(
    deps: DepsMut,
    env: Env,
...
pub fn create_pay_per_usage_policy(
    deps: DepsMut,
    env: Env,
    msg: InsurancePolicyData,
    info: MessageInfo,
    ...
    // Ensure that the sender is the owner of the contract
    //let state = STATE.load(deps.storage)?;
    if info.sender.to_string() != msg.policy.policy_holder {
        return Err(ContractError::Unauthorized {});
    }
    ...
    if avg_vss < 80 &&  avg_rpm < 2500 && mileage < 15000 {
        premium = premium / 2
    } 
